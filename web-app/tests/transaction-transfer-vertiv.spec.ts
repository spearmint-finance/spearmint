...
import { navigateTo, waitForDataGrid, searchDataGrid, waitForPageLoad } from "./utils/test-helpers";
...
test.describe("Transactions – VERTIV HOLDINGS reinvestment should be Transfer", () => {
  ...
  test("should display Transfer chip for VERTIV HOLDINGS reinvestment", async ({ page }) => {
    await searchDataGrid(page, "VERTIV HOLDINGS");
    await waitForPageLoad(page);

    // Find the row containing VERTIV HOLDINGS
    const row = page.locator('.MuiDataGrid-row', { hasText: /VERTIV HOLDINGS/i }).first();
