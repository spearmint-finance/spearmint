...
const updateBudget = async ({ id, data }: { id: number; data: any }) => {
                  ...
                  {editingBudgetId === item.budget_id ? (
                    <Box sx={{ display: "flex", gap: 1, mb: 1.5, alignItems: "center" }}>
                      <TextField
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && editAmount) {
                            updateMutation.mutate({ id: item.budget_id, data: { budget_amount: parseFloat(editAmount) } });
                          }
                      ...
                      <Button
                        ...
                        variant="contained"
                        onClick={() => {
                          if (editAmount) updateMutation.mutate({ id: item.budget_id, data: { budget_amount: parseFloat(editAmount) } });
                        }}
                        disabled={!editAmount || updateMutation.isPending}
                      >
                        Save
                      </Button>
                      <Button size="small" onClick={() => setEditingBudgetId(null)}>Cancel</Button>
                    </Box>
                  ) : (
                    <Box sx={{ mb: 1.5 }}>
                      <LinearProgress
