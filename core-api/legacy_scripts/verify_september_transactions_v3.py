...
print(df_excel_clean.nlargest(10, 'Amount', keep='all')[['Date', 'Description', 'Amount', 'Category']].to_string(index=False))

# Connect to database
print(f"\n=== DATABASE ANALYSIS ===")
conn = sqlite3.connect('financial_analysis.db')

# Check the correct table name for classifications
cursor = conn.cursor()
cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%class%';")
class_tables = cursor.fetchall()
print(f"Classification-related tables: {[t[0] for t in class_tables]}")

# Query transaction_classifications table
query_classifications = """
SELECT classification_id, name, exclude_from_expense_analysis, exclude_from_income_analysis
...
for idx in classification_summary.index:
    if pd.notna(idx):
        ...
        if len(class_info) > 0:
            classification_summary.loc[idx, 'name'] = class_info['name'].values[0]
            classification_summary.loc[idx, 'excluded'] = class_info['exclude_from_expense_analysis'].values[0]
    else:
...
if len(df_excluded) > 0:
    ...
    if len(df_excluded_class) > 0:
        ...
        for idx in excluded_class_breakdown.index:
            class_name = df_classifications[df_classifications['classification_id'] == idx]['name'].values[0]
...
print(f"✓ After filtering (excluding transfers & capital expenses), tool should show:")
