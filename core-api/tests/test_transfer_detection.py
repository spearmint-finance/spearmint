...
class TestTransferDetection:
    ...
    def test_transfer_category_detection_new_category(self, db_session):
        """Test that new 'Transfer' category is marked as transfer category."""
        ...
        try:
            ...
            assert category.is_transfer_category is True
            
            # Verify transaction was marked as transfer
    ...
    def test_transfer_category_case_insensitive(self, db_session):
        ...
        for i, category_name in enumerate(test_cases):
            ...
            try:
                ...
                # Should have created categories for each unique name
                # but all should be marked as transfer categories
                for cat in categories:
                    assert cat.is_transfer_category is True
    ...
    def test_non_transfer_category_not_marked(self, db_session):
        """Test that non-transfer categories are not marked as transfers."""
        service = ImportService(db_session)
        ...
        try:
            ...
            assert category is not None
            assert category.is_transfer_category is False
            
            # Verify transaction is NOT marked as transfer
            transaction = db_session.query(Transaction).first()
            assert transaction is not None
    ...
    def test_existing_transfer_category_updated(self, db_session):
        """Test that existing 'Transfer' category is updated if not marked."""
        # Create existing Transfer category without transfer flag
        existing_category = Category(
            category_name='Transfer',
            category_type='Expense',
            is_transfer_category=False  # Not marked
        )
        ...
        try:
            ...
            assert existing_category.is_transfer_category is True
    ...
    def test_multiple_transfers_in_one_import(self, db_session):
        ...
        try:
            ...
            assert categories[0].is_transfer_category is True
    ...
    def test_mixed_transfer_and_regular_transactions(self, db_session):
        ...
        try:
            ...
            transfers = db_session.query(Transaction).filter_by(is_transfer=True).all()
            assert len(transfers) == 2
            for tx in transfers:
                assert tx.include_in_analysis is False
            
            # Verify regular transaction is not marked as transfer
            regular = db_session.query(Transaction).filter_by(is_transfer=False).all()
